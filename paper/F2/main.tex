...
inference asks whether audio entered training \cite{flowleak2026,voxguard2026}, not whether
...
(Table~\ref{tab:pooled}). Per-speaker accuracies correlate weakly
across pairs (ECAPA Pearson $r$ .061/.291), so per-speaker difficulty is not a
stable speaker trait across pairs.
...
\begin{figure}[t]
\centering
\includegraphics[width=.80\linewidth]{fig_exp205_speakers.pdf}
...
comparison includes resynthesis effects. A post-hoc roster of fixed
non-verification readouts on the same grid stays well below the encoders: a
long-term spectral and cepstral average reaches .548/.549 and the best
mean-pooled self-supervised layer .633/.631. The carrier therefore
