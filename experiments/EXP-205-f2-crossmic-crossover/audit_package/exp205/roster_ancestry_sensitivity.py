...
script stratifies the EXP-205 result by whether each remaining speaker passed
...
tier-2-qualified stratum remains inside EXP-205.  No stratum changes the frozen
pooled verdict.
"""
...
PACKAGE_MODE = (EXP / "audit.json").is_file()
RESULT = EXP / "result.json" if PACKAGE_MODE else EXP / "audit_package/exp205/result.json"
GATE = EXP / "source/history/gate.json" if PACKAGE_MODE else EXP204 / "gate.json"
TIER1 = EXP / "source/history/pairs.json" if PACKAGE_MODE else EXP204 / "pairs.json"
TIER2 = EXP / "source/history/tier2_pairs.json" if PACKAGE_MODE else EXP204 / "tier2_pairs.json"
OUT = EXP / "roster_ancestry_sensitivity.json"
...
def main() -> int:
    ...
    output = {
        ...
        "question": "Does the EXP-205 direction persist across the observable residual strata of the prior tier-1 ECAPA gate?",
        "limitations": [
            "The tier-1 gate was ECAPA-based and the 30 kept speakers were the lexicographically first passing speakers.",
            "Three EXP-205 speakers were absent from the historical tier-1 gate record and are reported separately rather than assigned a status.",
            "All 24 tier-2-qualified speakers were consumed; EXP-205 is tier-2-ineligible by construction.",
            ...
            "No stratum can alter or rescue the frozen pooled EXP-205 verdict.",
        ],
