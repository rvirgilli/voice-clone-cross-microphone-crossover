...
def main() -> int:
    ...
    # Within-speaker presence-detection boundary (paper §5): recompute every printed point
    # from the released scores under the recorded cost model. The bootstrap intervals are
    # regenerated by code/presence_detection_sensitivity.py, which pins the same score file.
    presence = load("presence_detection_sensitivity.json")
    if presence.get("schema") != "exp205-postreview-presence-detection-sensitivity-v1":
        raise AssertionError("presence-detection schema invalid")
    if presence["pins"].get("scores.tsv") != sha256(DATA / "scores.tsv"):
        raise AssertionError("presence-detection result is not pinned to the released scores")
    cost = presence["cost_model"]
    if (cost["target_prior"], cost["threat_model_prior"], cost["cost_miss"], cost["cost_false_alarm"]) != (0.01, 0.5, 1.0, 1.0):
        raise AssertionError("presence-detection cost model changed")
    if presence["bootstrap"]["seed"] != result["counts"]["bootstrap_seed"]:
        raise AssertionError("presence-detection bootstrap seed differs from the frozen result")

    def error_rates(target: np.ndarray, nontarget: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        target, nontarget = np.sort(target), np.sort(nontarget)
    ...
    def normalized_min_dcf(target: np.ndarray, nontarget: np.ndarray, prior: float) -> float:
        miss, false_alarm = error_rates(target, nontarget)
        return float((prior * miss + (1.0 - prior) * false_alarm).min() / min(prior, 1.0 - prior))

    for prompt_mic, direction in (("mic1", DIRECTIONS[0]), ("mic2", DIRECTIONS[1])):
        for encoder in ("ecapa", "wavlm"):
            target, nontarget = [], []
            for row in rows:
                if row["prompt_mic"] != prompt_mic:
                    continue
                own, other = row["seed_arm"], "B" if row["seed_arm"] == "A" else "A"
                target.append(float(row[f"{encoder}_{own}"]))
                nontarget.append(float(row[f"{encoder}_{other}"]))
            target, nontarget = np.asarray(target), np.asarray(nontarget)
            if target.size != 1728 or nontarget.size != 1728:
                raise AssertionError("presence-detection trial census invalid")
            cell = presence["directions"][direction][encoder]
            close(equal_error_rate(target, nontarget), cell["eer"], f"presence EER {direction}/{encoder}")
            close(normalized_min_dcf(target, nontarget, 0.01), cell["ndcf"], f"presence minDCF {direction}/{encoder}")
            close(normalized_min_dcf(target, nontarget, 0.5), cell["ndcf_threat_prior"], f"presence minDCF@.5 {direction}/{encoder}")
